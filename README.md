...
貨物データから、**必要本数の見積り**、**本数固定での積載検証**、**縮尺ボックスによる手動バン詰め**を行うコンテナ積載計画（Vanning Plan）アプリです。公開用のGitHub Pages版と、計算仕様の参照実装であるPython + Streamlit版を収録しています。
...
- 必要本数の自動見積り、指定本数でのSTANDARD / SPECIAL検証、指定コンテナへの手動配置
- 手動配置からコンテナ別集計・CLP・CSV・Excel・印刷帳票を生成
- OOG・入口通過・RF / OT / FR・在来船検討の判定
- 上面配置図、Payload、風袋込み総重量、偏荷重、重量集中の監査、コンテナ間の重量均等化
- CSV、Excel帳票、印刷 / PDF保存（コンテナ別・全体総計を含む）
- プラン一式を圧縮した共有URL・QRコードの生成と復元（高密度時は読取可能な密度へ自動分割）
- PC・タブレット・スマートフォン対応
...
- 標準コンテナ: 大型貨物をトラックヘッド側（x=0）から配置し、床面の残りに入る後続貨物をすべて試してから段積みへ進む
- 隣接高さ: 高さ差100cmは注意目安とし、配置不可にはしない。トラック側から背の高い順になるよう配置し、差が出る箇所は固縛・倒れ込み対策の確認対象とする
- 入口余裕: 判定時に入口寸法マージンとして**幅1cm・高さ3cm**を確保
- OOG算出: 参照コンテナ（標準は40HC）との差分から超過量を計算
  - `OW(each)`: ピース単位の幅超過
  - `OH`: 高さ超過

### 4.3 コンテナ選定（20GP小口判定・40HC基準・40GP代用判定）

標準コンテナは以下の方針で選定します。

1. **20GP小口判定**: 全標準貨物の総容積・総重量が20GP容量内で、かつ20GP 1本に実配置できる場合は、余分な40FTスペースを避けて20GPを選定。判定値は結果の選定根拠へ表示
2. **40HC基準**: 20GP 1本に収まらない標準貨物は40HCを基準に本数と配置を算出
3. **40GP代用判定**: 40HCの各バンプランについて、同じ配置のまま40GPの内寸・入口寸法・最大積載重量を満たすかを表示
...
4. 40FR制約
   - 40FR内は段積み禁止
   - 容積2m3以下は40FR候補から除外
   - 通常コンテナの大型順を必須とせず、デッキ中心に対する点対称性と貨物重量重心を評価して中央寄せする
