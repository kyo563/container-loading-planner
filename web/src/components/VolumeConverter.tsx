...
export function VolumeConverter() {
  ...
  const metrics = [
    ...
    { label: "航空容積重量", value: fmt(m3 * CW_KG_PER_M3, 1), unit: "kg", note: "1m³ = 166.67kg" },
  ...
  return (
    <main className="page-shell converter-page">
      <div className="page-intro">
        ...
        <h1>容積・物流単位換算</h1>
        <p>cmディメンションまたはm³から、才数・航空容積重量・RT・MTを同時換算します。</p>
      </div>
      <section className="panel converter-panel">
        ...
        <div className="conversion-results">
          {metrics.map((metric, index) => (
            <div key={metric.label} className={index === 0 ? "conversion-card primary" : "conversion-card"}>
              <small>{metric.label}</small><strong>{metric.value}<em>{metric.unit}</em></strong><p>{metric.note}</p>
            </div>
          ))}
        </div>
        <div className="formula-note"><strong>換算上の注意</strong><p>運賃計算ではキャリアや輸送モードごとに端数処理・最低重量・換算係数が異なる場合があります。最終的には適用タリフを確認してください。</p></div>
      </section>
    </main>
  );
}
